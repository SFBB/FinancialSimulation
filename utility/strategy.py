...
class investment_record(ABC):
    ...
    def handle_choice(
        ...
        if choice == buy_or_sell_choice.Buy:
            ...
            comm = max(
                asset_value * self.market_config.commission_rate,
                self.market_config.min_commission,
            )
            total_cost = comm
        elif choice == buy_or_sell_choice.Sell:
            comm = max(
...
class strategy_base(ABC):
    ...
    def tick(
        self,
        latest_stocks_info: dict[str, stock_info],
        latest_economics_info: dict[str, economic_info_base],
        ...
        if all_prices_valid:
            self.__investments_info__.record_daily_equity(
                self.today_time, current_equity
            )
        ...
        for ticket in choice.keys():
            ...
            if self.market_config.execution_mode == ExecutionMode.NEXT_OPEN:
                ...
                self.queued_orders.append(
                    {"ticket": ticket, "action": action, "amount": amount}
                )
    ...
    def _execute_trade(self, ticket, action, amount, execution_price=None):
        ...
        if action == buy_or_sell_choice.Sell and self.market_config.settlement == "T+1":
            ...
            if amount > available:
                ...
                print(
                    f"Refusing to sell frozen shares for {ticket}. Requested: {amount}, Available: {available}"
                )
                amount = available
                if amount <= 0:
        ...
        if self.market_config.volume_limit_pct < 1.0:
            ...
            if daily_vol > 0:
                ...
                if amount > max_shares:
                    print(
                        f"Liquidity Crunch for {ticket}: Requested {amount} shares, but Day Volume was {daily_vol}. Capping at {max_shares:.2f} (10% limit)."
                    )
                    amount = max_shares
        ...
        if action != buy_or_sell_choice.DoNothing:
            ...
            if (
                action == buy_or_sell_choice.Buy
                and self.market_config.settlement == "T+1"
            ):
                if ticket not in self.frozen_stock:
                    self.frozen_stock[ticket] = 0
    ...
    def print_performance_report(self):
        ...
        mkt_type = (
            "US (T+0)"
            if self.market_config.market_type == Market.US
            else "A-Shares (CN) [T+1 Settlement]"
        )
        ...
        if stats:
            print(
                f"Period:            {stats.get('Start Date', 'N/A')} to {stats.get('End Date', 'N/A')}"
            )
