...
class Tech_Trinity_Momentum_Strategy(strategy_base):
    """
    Tech Trinity Portfolio V2 (Adaptive Momentum Aggressive)
    ------------------------------------------------------
    The "Winner Takes All" version.
    
    1. Allocation: Dynamic. Invests only in Top K assets sorted by Momentum.
    2. Logic: Pure Price Action (Trend + Volatility).
       - Ignores Fundamental Chowder Score (assumes Price leads Fundamentals).
    3. Macro: Global risk ceiling.
    
    Pros: High Alpha, drops losers quickly.
    Cons: Higher turnover, higher drawdown than V1.
    """
    ...
    def get_name(self):
        return f"Portfolio_V2_Momentum_Top{self.top_k}of{len(self.stock_names)}"
